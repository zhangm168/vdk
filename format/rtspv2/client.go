		...
		if i2.AVType == VIDEO {
			if i2.Type == av.H264 {
				if len(i2.SpropParameterSets) > 1 {
					if len(i2.SpropParameterSets[0]) > 3 && len(i2.SpropParameterSets[1])>1 {
						...
						client.Println("SDP Video Codec SDP is empty, ignore ")
			...
			if client.videoCodec == av.H265 {
				...
				case naluType == 24:// STAP-A
					...
					for true {
						...
						nalSize := int(binary.BigEndian.Uint16(content[offset1:offset1+2]))
						if size < (nalSize + 2) {
						...
						retmap = append(retmap, &av.Packet{
							Data:            append(binSize(nalSize), content[offset1+2:offset1+2+nalSize]...),
							CompositionTime: time.Duration(1) * time.Millisecond,
							Duration:        time.Duration(float32(timestamp-client.PreVideoTS)/90) * time.Millisecond,
							Idx:             client.videoIDX,
							IsKeyFrame:      false,
